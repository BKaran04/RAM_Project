...
ignore_bins invalid = binsof(WRITE) intersect {1} && binsof(READ) intersect {1};
} 
endgroup
...
repeat(1) @(vif.drv_cb);
$display("Driver write operation driving data_in=%0h,write_enb=%0d,read_enb=%0d,address=%0h,T=%0t",vif.drv_cb.data_in,vif.drv_cb.write_enb,vif.drv_cb.read_enb,vif.drv_cb.address,$time);
vif.drv_cb.write_enb<=0;
mbx_dr.put(drv_trans);
drv_cg.sample();
...
$display("Reference Model Data out from memory data_out=%0h,T=%0t",
ref_trans.data_out,$time);
end
